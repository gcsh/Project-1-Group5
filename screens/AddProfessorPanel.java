...
public class AddProfessorPanel  extends JPanel {
	...
	public AddProfessorPanel(final JFrame currentGUIFrame) {
			...
			public void actionPerformed(ActionEvent e) {
				if(firstName!=null && firstName.getText().trim().length()!=0 && lastName!=null && lastName.getText().trim().length()!=0
					...
					try{
						...
						String [] record = {lastName.getText(),firstName.getText(),gradSchool.getText(),degree.getText(),title.getText(),listData[0],fallLoad.getText(),springLoad.getText(),summerLoad.getText()};
		...
		addNewProfessor.setBounds(350, 440, 125, 30);
