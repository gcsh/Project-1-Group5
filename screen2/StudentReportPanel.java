...
public class StudentReportPanel extends JPanel {
	...
	public StudentReportPanel(final JFrame currentGUIFrame) {
		setLayout(null);		
		...
		lblNewLabel = new JLabel("Student-Course Report");
		...
		Object columnNames[] = { "Student", "Number of courses completed", "Number of courses needed"};
		DatabaseHelper helper=new DatabaseHelper();
		LinkedHashMap studentMap=helper.getAllStudentDataFromStudentDumpAndForecast();
		LinkedHashMap studentCourseMap=helper.getAllStudentCourseDataMap();
		if(studentMap!=null && studentMap.size()!=0){
			rowData=new Object[studentMap.size()][4];
			...
			while (iterator.hasNext()) {
				...
				if(((ArrayList)studentCourseMap.get(entry.getKey()))!=null){
					int coursesCount=((ArrayList)studentCourseMap.get(entry.getKey())).size();
					if(coursesCount>10){
						rowData[count][increment]=((ArrayList)studentCourseMap.get(entry.getKey())).size();
						increment=increment+1;
						rowData[count][increment]="0";
					}else{
						rowData[count][increment]=((ArrayList)studentCourseMap.get(entry.getKey())).size();
						increment=increment+1;
						rowData[count][increment]=""+(10-((ArrayList)studentCourseMap.get(entry.getKey())).size());
					}
					
				}else{
					rowData[count][increment]="0";
					increment=increment+1;
					rowData[count][increment]="10";
				}
